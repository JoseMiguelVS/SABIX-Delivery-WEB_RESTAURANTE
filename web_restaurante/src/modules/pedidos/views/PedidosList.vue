      ...
      <div v-for="pedido in pedidos" :key="pedido.id" @click="goToDetail(pedido.id)" class="order-card" :style="{
          ...
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              ...
              <div style="font-size: 13px; color: #666; margin-top: 4px;">
                {{ pedido.items_count || 0 }} productos
        ...
        <div
          ...
          <template v-if="pedido.status === 'ready'">
            <button @click.stop="openAssignModal(pedido)" class="btn-action btn-assign-delivery" :style="{
              background: assignHover ? '#2563eb' : '#3b82f6',
              padding: '6px 16px',
              color: 'white',
              border: 'none',
              borderRadius: '8px',
            ...
            <!-- Ver repartidor si ya tiene asignado -->
...
import PedidoStatusModal from './PedidoStatusModal.vue';
import AssignDeliveryModal from '../components/AssignDeliveryModal.vue';

defineOptions({ name: 'PedidosList' });
...
const handleModalConfirm = async (data) => {
  ...
  // Si el estado es "ready" y la actualización fue exitosa, abrir el modal de asignación
  if (status === 'ready' && successResult) {
    setTimeout(() => {
      const pedidoActualizado = pedidos.value.find(p => p.id === selectedPedido.value.id);
      if (pedidoActualizado && pedidoActualizado.status === 'ready') {
        openAssignModal(pedidoActualizado);
      }
    }, 1000);
  }
...
// Watch para detectar pedidos listos
watch(
  () => pedidos.value,
  (newPedidos) => {
    const readyPedidos = newPedidos.filter(
      p => p.status === 'ready' && !p.delivery_person_id
    );
    if (readyPedidos.length > 0 && !showAssignModal.value && !showStatusModal.value) {
      // Hay pedidos listos para asignar repartidor
    }
  },
  { deep: true }
);
