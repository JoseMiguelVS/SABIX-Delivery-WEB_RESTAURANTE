<template>
  <div style="padding: 24px; max-width: 1200px; margin: 0 auto;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
      <div>
        <h1 style="font-size: 24px; font-weight: 700; color: #1a1a1a; margin-bottom: 4px;">Productos</h1>
        <p style="font-size: 14px; color: #666;">Gestiona el catálogo de productos</p>
      </div>
      <button @click="goToCreate"
        ...
        :style="buttonHover ? { background: '#6a086a', transform: 'translateY(-1px)', boxShadow: '0 4px 12px rgba(126,9,126,0.3)' } : {}">
    ...
    <div v-else
      ...
      <div v-if="productos.length === 0" style="text-align: center; padding: 60px 20px; color: #94a3b8;">
        ...
        <p style="font-size: 14px;">Comienza creando tu primer producto</p>
      </div>
      ...
      <table v-else style="width: 100%; border-collapse: collapse; font-size: 14px;">
        ...
        <tbody>
          <tr v-for="producto in productos" :key="producto.id"
            style="border-bottom: 1px solid #f1f5f9; transition: background 0.15s;" @mouseenter="hoverRow = producto.id"
            @mouseleave="hoverRow = null" :style="hoverRow === producto.id ? { background: '#f8fafc' } : {}">
            <td style="padding: 12px 20px;">
              <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                  ...
                  <div style="font-size: 13px; color: #666;">{{ producto.description || 'Sin descripción' }}</div>
                </div>
              </div>
            </td>
            ...
            <td style="padding: 12px 20px; font-weight: 600; color: #1a1a1a;">
              ${{ producto.price }}
            ...
            <td style="padding: 12px 20px;">
              <span style="font-weight: 600;"
                :style="producto.stock > 10 ? { color: '#22c55e' } : producto.stock > 0 ? { color: '#f59e0b' } : { color: '#ef4444' }">
                {{ producto.stock || 0 }}
              </span>
            </td>
            <td style="padding: 12px 20px;">
            ...
            <td style="padding: 12px 20px; text-align: center;">
              <div style="display: flex; align-items: center; justify-content: center; gap: 4px;">
                ...
                <button @click="goToEdit(producto.id)"
                  ...
                  :style="editHover === producto.id ? { color: '#7e097e', background: '#fa8bf5' } : {}" title="Editar">
                  <svg style="width: 18px; height: 18px;" viewBox="0 0 20 20" fill="currentColor">
                    <path
